...
import {
  ...
  Instagram, Link as LinkIcon, CheckCircle2, Copy, Download, QrCode
} from 'lucide-react';
...
const PHOTOGRAPHER_SHARE = 1 - QPAY_FEE - PLATFORM_FEE - OWNER_COMMISSION_SHARED;
const OWNER_COMMISSION_OWNED = 1 - QPAY_FEE - PLATFORM_FEE; // 0.96

// ─── Sample photo for live preview ───────────────────────────────────────────
...
export default function CreateAlbumPage() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      // Build watermark_value: text string or base64 image
      let watermarkValue = '';
      if (wmType === 'text') {
        watermarkValue = JSON.stringify({
          text: wmText,
          fontSize: wmFontSize,
      ...
      const ownerCommission = revenueModel === 'owned' ? OWNER_COMMISSION_OWNED : OWNER_COMMISSION_SHARED;
      ...
      // Also save contact info to users table
      if (Object.values(contactInfo).some(v => v)) {
        await supabase.from('users').update({ contact_info: contactInfo }).eq('id', user!.id);
      }
  ...
  function downloadQR() {
    if (!successData) return;
    const qrImageUrl = `https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${encodeURIComponent(successData.shareLink)}`;
          ...
          <div className="lg:col-span-2 space-y-8">
              ...
              <div className="space-y-5">
                <Field label="Цомгийн нэр" error={errors.name} required>
                  <input
                    ...
                    placeholder="e.g. Wedding of Bat & Enkhjin"
                ...
                <div className="grid grid-cols-2 gap-4">
                  <Field label="Арга хэмжээний огноо" error={errors.eventDate} required>
                    <input
                      ...
                      className={inputClass(!!errors.eventDate) + ' [color-scheme:dark]'}
                    />
                  </Field>
                ...
                <Field label="Тайлбар">
                  <textarea
                    ...
                    placeholder="Зурагчидад энэ арга хэмжээний талаар хэлэх..."
                    rows={3}
            ...
            <Section title="Орлогын загвар" icon={<span className="text-xs font-bold text-stone-400">%</span>}>
              <div className="space-y-4">
                <div className="flex rounded-xl overflow-hidden border border-white/10">
                  <TypeToggleBtn
                    active={revenueModel === 'shared'}
                    onClick={() => setRevenueModel('shared')}
                    icon={<span className="text-xs font-bold">86/10</span>}
                    label="Хуваалцах загвар"
                  />
                  <TypeToggleBtn
                    active={revenueModel === 'owned'}
                    onClick={() => setRevenueModel('owned')}
                    icon={<span className="text-xs font-bold">96</span>}
                    label="Бүрэн өмчлөх загвар"
                  />
                </div>
                <div className={`rounded-xl p-4 border text-sm ${
                  ...
                  {revenueModel === 'shared' ? (
                    <p>Зурагчид борлуулалтын <strong>86%</strong>, зохион байгуулагч <strong>10%</strong> авна. Борлуулалт бүрийн дараа тооцоо хийх шаардлагатай.</p>
                  ) : (
                  ...
                  <div className="space-y-4">
                    <Field label="Усан тэмдгийн текст" error={errors.watermarkValue} required>
                      <input
                        ...
                        className={inputClass(!!errors.watermarkValue)}
                      />
                    </Field>
                    ...
                    {wmImagePreview ? (
                      <div className="relative inline-block">
                        <img
                          src={wmImagePreview}
                          alt="Watermark logo"
                          className="h-20 rounded-lg border border-white/10 object-contain bg-white/5 px-4"
                        />
                        <button
                          type="button"
                          onClick={() => { setWmImageFile(null); setWmImagePreview(''); }}
                      ...
                      <button
                        ...
                        className="w-full border-2 border-dashed border-white/20 hover:border-amber-500/50 hover:bg-amber-500/5 rounded-xl py-8 flex flex-col items-center gap-3 text-stone-400 hover:text-amber-400 transition-all duration-200"
                      >
                  ...
                  <div className="grid grid-cols-3 gap-1.5 w-fit mt-1">
                    {POSITIONS.map(p => (
                      <button
                        ...
                        className={`w-12 h-12 rounded-lg border transition-all duration-150 flex items-center justify-center ${
                          wmPosition === p.value
                      ...
                      >
                        <div className={`w-2 h-2 rounded-full ${
                          wmPosition === p.value ? 'bg-stone-950' : 'bg-white/40'
                        }`} />
                      </button>
                    ))}
                  </div>
              ...
              <div className="space-y-5">
                ...
                <div className="flex rounded-xl overflow-hidden border border-white/10">
                  <TypeToggleBtn
                    ...
                    icon={<span className="text-xs font-bold">FREE</span>}
                    label="Үнэгүй татах"
                  />
                  <TypeToggleBtn
                    active={!isFree}
                  ...
                  <Field label="Татах үнэ (₮)" error={errors.downloadPrice} required>
                    <div className="relative">
                      ...
                      <input
                        ...
                        placeholder="10000"
                        min={1}
                        className={inputClass(!!errors.downloadPrice) + ' pl-8'}
                      />
                    </div>
                  </Field>
                )}
                ...
                <FeeBreakdown price={isFree ? 0 : parseFloat(downloadPrice) || 0} revenueModel={revenueModel} />
              </div>
            </Section>
            ...
            <Section title="Холбоо барих мэдээлэл" icon={<Phone className="w-4 h-4" />}>
              <p className="text-stone-500 text-sm mb-4">
                Таны цомогт нэгдэх зурагчидад харагдана.
              </p>
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
          ...
          <div className="space-y-6">
            ...
            <div className="sticky top-24">
              <div className="bg-white/5 border border-white/10 rounded-2xl p-4">
                ...
                <div className="relative rounded-xl overflow-hidden aspect-[3/2]">
                  ...
                  <div className={`absolute flex ${overlayPositionClass[wmPosition]} pointer-events-none`}>
                    {wmType === 'text' && wmText && (
                      <span
                        className="font-semibold px-1 select-none whitespace-nowrap"
                        style={{
                          fontSize: `${Math.max(8, Math.round(wmFontSize * 0.4))}px`,
                          color: wmColor,
                          opacity: wmOpacity / 100,
                ...
                <p className="text-stone-600 text-xs mt-2 text-center">
                  Худалдан авагчид ийм байдлаар усан тэмдэгтэй зурагнуудыг харна
                </p>
              </div>
              ...
              <button
                type="button"
                onClick={() => navigate('/dashboard')}
                className="w-full mt-3 text-stone-400 hover:text-white transition-colors py-2 text-sm"
              >
                Цуцлах
              </button>
            </div>
...
function TypeToggleBtn({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`flex-1 flex items-center justify-center gap-2 py-2.5 px-3 text-sm font-medium transition-all duration-200 ${
        active
...
function FeeBreakdown({ price, revenueModel = 'shared' }: { price: number; revenueModel?: 'shared' | 'owned' }) {
  if (price <= 0) {
    return (
      <div className="bg-white/5 rounded-xl p-4 text-stone-500 text-sm text-center">
        Шимтгэлийн задаргааг харахын тулд үнэ оруулна уу
      </div>
    );
  }

  const qpayFee   = Math.round(price * QPAY_FEE);
  const platform  = Math.round(price * PLATFORM_FEE);

  const rows = revenueModel === 'owned'
    ? [
        { label: 'QPay шимтгэл (1%)',       amount: qpayFee,                              color: 'text-stone-400', bar: 'bg-stone-600', pct: 1  },
        { label: 'Платформ шимтгэл (3%)',   amount: platform,                             color: 'text-stone-300', bar: 'bg-stone-500', pct: 3  },
        { label: 'Таны хувь (96%)',          amount: Math.round(price * OWNER_COMMISSION_OWNED), color: 'text-amber-400', bar: 'bg-amber-500', pct: 96 },
      ]
    : [
        { label: 'QPay шимтгэл (1%)',       amount: qpayFee,                              color: 'text-stone-400', bar: 'bg-stone-600',  pct: 1  },
        { label: 'Платформ шимтгэл (3%)',   amount: platform,                             color: 'text-stone-300', bar: 'bg-stone-500',  pct: 3  },
        { label: 'Таны хувь (10%)',          amount: Math.round(price * OWNER_COMMISSION_SHARED), color: 'text-amber-400', bar: 'bg-amber-500',  pct: 10 },
        { label: 'Зурагчид (86%)',           amount: Math.round(price * PHOTOGRAPHER_SHARE), color: 'text-green-400', bar: 'bg-green-500',  pct: 86 },
      ];

  return (
    <div className="bg-white/5 border border-white/10 rounded-xl p-4 space-y-3">
      <p className="text-stone-300 text-sm font-medium">₮{price.toLocaleString()} үнийн шимтгэлийн задаргаа</p>
      {rows.map(r => (
        <div key={r.label}>
          <div className="flex justify-between text-xs mb-1">
            <span className="text-stone-400">{r.label}</span>
            <span className={r.color + ' font-medium'}>₮{r.amount.toLocaleString()}</span>
          </div>
...
function SuccessModal({
  shareLink, copied,
  onCopy, onDownloadQR, onGoToAlbum, onClose,
}: {
  shareLink: string;
  copied: boolean;
  onCopy: () => void;
      ...
      <div className="bg-stone-900 border border-white/10 rounded-2xl w-full max-w-md shadow-2xl">
        <div className="p-6 text-center border-b border-white/10">
          ...
          <p className="text-stone-400 text-sm mt-1">
            Холбоос эсвэл QR кодыг зурагчид болон худалдан авагчидтай хуваалцаарай.
          </p>
        </div>
        ...
        <div className="p-6 space-y-5">
          ...
          <div className="bg-white/5 border border-white/10 rounded-xl flex items-center gap-2 px-3 py-2.5 overflow-hidden">
            ...
            <button
              onClick={onCopy}
              className="flex items-center gap-1.5 text-xs font-medium text-amber-400 hover:text-amber-300 transition-colors flex-shrink-0"
            >
          ...
          <div className="grid grid-cols-2 gap-3">
            <button
              ...
              className="flex items-center justify-center gap-2 bg-white/5 hover:bg-white/10 border border-white/10 text-white rounded-xl py-2.5 text-sm font-medium transition-all duration-200"
            >
            ...
            <button
              onClick={onGoToAlbum}
              className="flex items-center justify-center gap-2 bg-amber-500 hover:bg-amber-400 text-stone-950 rounded-xl py-2.5 text-sm font-semibold transition-all duration-200"
            >
              Цомог харах
            </button>
          </div>
...
function inputClass(hasError: boolean) {
  ...
  } focus:ring-2 text-white placeholder-stone-600 rounded-xl px-4 py-3 outline-none transition-all duration-200 [&_option]:bg-stone-900`;
}
