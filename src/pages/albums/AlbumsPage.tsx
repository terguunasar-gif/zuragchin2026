...
export default function AlbumsPage() {
  ...
  useEffect(() => {
    ...
    setFiltered(
      q ? albums.filter(a => a.name.toLowerCase().includes(q) || a.organizer_name.toLowerCase().includes(q)) : albums,
    );
  }, [search, albums]);
  ...
  const statusTag = (albumId: string) => {
    ...
    return (
      <button
        onClick={() => openJoinModal(filtered.find(a => a.id === albumId)!)}
        className="flex items-center gap-1.5 text-xs font-semibold text-stone-950 bg-amber-500 hover:bg-amber-400 px-3 py-1.5 rounded-lg transition-colors"
      >
        ...
        {loading ? (
          ...
          <div className="text-center py-24">
            ...
            <p className="text-stone-500">
              {search ? 'Өөр хайлтын үгээр оролдоно уу.' : 'Одоогоор идэвхтэй цомог байхгүй байна.'}
            </p>
          </div>
        ) : (
...
function AlbumCard({
  ...
  return (
    <div className="bg-white/5 border border-white/10 hover:border-white/20 rounded-2xl p-5 flex flex-col gap-4 transition-all duration-200 group">
      ...
      {album.description && (
        <p className="text-stone-400 text-sm leading-relaxed line-clamp-2">{album.description}</p>
      )}
      ...
      <div className="flex items-center justify-between pt-1 border-t border-white/5">
        <span className="text-stone-500 text-xs">
          {album.is_free ? (
            <span className="text-green-400 font-medium">Үнэгүй татах</span>
          ) : (
            <span>₮{album.download_price.toLocaleString()} / photo</span>
          )}
        </span>
        {statusTag}
      </div>
      ...
      <div className="relative bg-stone-900 border border-white/10 rounded-2xl w-full max-w-md shadow-2xl">
        ...
        <div className="px-6 py-5 space-y-5">
          ...
          <div className="bg-white/5 rounded-xl p-4 space-y-2">
            ...
            {album.description && (
              <p className="text-stone-400 text-sm leading-relaxed pl-12">{album.description}</p>
            )}
            <div className="flex gap-4 pl-12 text-xs text-stone-500">
              <span className="flex items-center gap-1">
                <Image className="w-3.5 h-3.5" />
                {album.photo_count} зураг байршуулсан
              </span>
              <span>
                {album.is_free ? (
                  <span className="text-green-400">Үнэгүй татах</span>
                ) : (
                  <span>₮{album.download_price.toLocaleString()} / зураг</span>
                )}
              </span>
            </div>
          </div>
          ...
          <div>
            ...
            {loadingOrganizer ? (
              <div className="flex items-center gap-2 text-stone-500 text-sm">
                <div className="w-4 h-4 border-2 border-white/20 border-t-amber-500 rounded-full animate-spin" />
                Уншиж байна...
              </div>
            ) : organizer ? (
          ...
          {success ? (
            <div className="flex flex-col items-center text-center py-4 gap-3">
              ...
              <p className="text-stone-400 text-sm">
                Зохион байгуулагч таны хүсэлтийг хянах болно. Хяналтын самбар дээр төлөвийг харна уу.
              </p>
              <button
                onClick={onClose}
                className="mt-1 bg-amber-500 hover:bg-amber-400 text-stone-950 font-semibold px-6 py-2.5 rounded-xl transition-colors text-sm"
              >
                Болсон
              </button>
            </div>
            ...
            <div className="flex gap-3 pt-1">
              <button
                onClick={onClose}
                className="flex-1 bg-white/5 hover:bg-white/10 border border-white/10 text-white font-medium px-4 py-2.5 rounded-xl transition-colors text-sm"
              >
                Цуцлах
              </button>
              <button
                onClick={onConfirm}
                disabled={submitting}
                className="flex-1 bg-amber-500 hover:bg-amber-400 disabled:opacity-50 disabled:cursor-not-allowed text-stone-950 font-semibold px-4 py-2.5 rounded-xl transition-colors text-sm flex items-center justify-center gap-2"
              >
                {submitting ? (
